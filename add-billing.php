...
if(isset($_POST['submit']))
{
  ...
  {
      $sql ="INSERT INTO billing(patientid,appointmentid,discount,taxamount) values('$_POST[patientid]','$_POST[appointmentid]','$_POST[discount]','$_POST[taxamount]')";
        ...
        <div class="popup popup--icon -success js_success-popup popup--visible">
          ...
          <div class="popup__content">
            ...
            <p>
             ...
             <?php echo "<script>setTimeout(\"location.href = 'payment_billing.php';\",1500);</script>"; ?>
            </p>
          </div>
        </div>
<?php
...
<li class="breadcrumb-item"><a>Billing Record</a>
</li>
</li>
</ul>
</div>
...
<?php
    ...
    $sqldoctor = "SELECT * FROM doctor WHERE doctorid='$rs[doctorid]'";
	  ...
	  $sqlappointment = "SELECT * FROM appointment WHERE patientid='$rs[patientid]'";
...
<form method="post" action="" name="frmpresrecord" onSubmit="return validateform()"> 
  <input type="hidden" name="patientid" value="<?php echo  $rspatient['patientid']; ?>"  />
  <input type="hidden" name="appointmentid" value="<?php echo  $rsappointment['appointmentid']; ?>"  />
    <div class="table-responsive dt-responsive">
    <table id="" class="table table-striped table-bordered nowrap">
      <tbody>
        ...
        <tr>
          <td>Tax Amount</td>
          <td><input class="form-control" type="number" name="taxamount" id="taxamount" style="background-color:pink;" />
		 </td>
        </tr>
        <tr>
          <td>Discount</td>
          <td><input class="form-control" type="number" min="1" name="discount" id="discount" onchange="" /></td>
        </tr>
  ...
  <tr>
    ...
    while($rs = mysqli_fetch_array($qsql))
    {
        ...
        <td>&nbsp;$rs[dosage]</td>
          <td>&nbsp;₹$rs[cost]</td>
       <td>&nbsp;$rs[unit]</td>
       <td  align='right'>Ksh." . $rs['cost'] * $rs['unit'] . "</td>";
      if(!isset($_SESSION['patientid']))
      {
    ...
    ?>
      ...
      <th align="right">Ksh.<?php echo $gtotal; ?></th>
      <td></td>
          </tr>
        <tr>
