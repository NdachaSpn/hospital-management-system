...
<form method="post" action="" name="frmpresrecord" onSubmit="return validateform()"> 
  <input type="hidden" name="prescriptionid" value="<?php if(isset($_GET['prescriptionid'])) { echo $_GET['prescriptionid']; } ?>"  />
    <div class="table-responsive dt-responsive">
    <table id="" class="table table-striped table-bordered nowrap">
      <tbody>
    ...
    {
      echo "<option value='$rsmedicine[medicineid]'>$rsmedicine[medicinename] ( Ksh. $rsmedicine[medicinecost] )</option>";
    }
    ?>
      </select>
      </td>
        ...
        <tr>
          <td>Cost</td>
          <td><input class="form-control" type="text" name="cost" id="cost" value="<?php echo $rsmedicine['medicinecost']; ?>" style="background-color:pink;" /></td>
        </tr>
        <tr>
          <td>Unit</td>
          <td><input class="form-control" type="number" min="1" name="unit" id="unit" onkeyup="calctotalcost(cost.value,this.value)" onchange="" /></td>
        </tr>
        <tr>
          <td>Total Cost</td>
